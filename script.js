...
function openModal(product) {
    ...
    if (product.status_text) {
        detailHtml += `<span class="modal-detail-pill" ${product.status_color ? `style="color:${product.status_color};border-color:${product.status_color}44;"` : ''}>
            ${escapeHtml(product.status_text)}
        </span>`;
    }
    ...
    if (variants.length > 1) {
        ...
        variants.forEach((v, i) => {
            const vPrice = parseFloat(v.price).toFixed(2);
            customHtml += `<option value="${v.id}" data-price="${v.price}" data-price-slash="${v.price_slash || ''}" ${i === 0 ? 'selected' : ''}>
                ${escapeHtml(v.name)} — $${vPrice}
            </option>`;
        });
        customHtml += `
            </select>
    ...
    // Purchase Link — Always enabled no matter what
    const firstVariant = variants[0];
